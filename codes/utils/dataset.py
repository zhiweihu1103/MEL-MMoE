...
class DataModuleForMMoE(pl.LightningDataModule):
    def __init__(self, args):
        super(DataModuleForMMoE, self).__init__()
        self.args = args
        current_directory = os.path.dirname(os.path.abspath(__file__))
        base_path = current_directory[0:current_directory.rfind('/')]
        self.base_path = base_path[0:base_path.rfind('/')]
        self.tokenizer = CLIPProcessor.from_pretrained(self.base_path + self.args.pretrained_model).tokenizer
        self.image_processor = CLIPProcessor.from_pretrained(self.base_path + self.args.pretrained_model).feature_extractor
        with open(self.base_path + self.args.data.qid2id, 'r', encoding='utf-8') as f:
            self.qid2id = json.loads(f.readline())
        self.raw_kb_entity = sorted(_load_json_file(self.base_path + self.args.data.entity), key=lambda x: x['id'])
        self.kb_entity = self.setup_dataset_for_entity(self.base_path + self.args.data.entity, self.raw_kb_entity)
        self.kb_id2entity = {raw_ent['id']: ent for raw_ent, ent in zip(self.raw_kb_entity, self.kb_entity)}

        self.train_data = self.setup_dataset_for_mention(self.base_path + self.args.data.train_file, _load_json_file(self.base_path + self.args.data.train_file))
        self.val_data = self.setup_dataset_for_mention(self.base_path + self.args.data.dev_file, _load_json_file(self.base_path + self.args.data.dev_file))
        self.test_data = self.setup_dataset_for_mention(self.base_path + self.args.data.test_file, _load_json_file(self.base_path + self.args.data.test_file))
    ...
    def setup_dataset_for_mention(self, path, data):
        ...
        for sample_dict in tqdm(data, desc='PreProcessing'):
            sample_type = 1
            entity, mention, text, desc = unquote(sample_dict.pop('entities')), unquote(sample_dict.pop('mentions')), sample_dict.pop('sentence'), sample_dict.pop('desc')
        ...
        if len(img_list):
            ...
            try:
                img_path = os.path.join(
                    self.base_path + self.args.data.kb_img_folder if sample_type == 0 else self.base_path + self.args.data.mention_img_folder,
                    img_name)
